...
class PuppetWork extends Puppet {
	...
	messageContact(messageId: string): Promise<string> {
        throw new Error("messageContact Method not implemented.")
    }
    ...
    public async messageFile(messageId: string): Promise<FileBox> {
		...
		switch (content.msgtype){
			...
			case "meeting_voice_call":
				result = libSDK.getMedia(0, content.meeting_voice_call.sdkfileid, 
				"cache/"+content.msgid+".amr", "None");
				filebox = FileBox.fromFile("cache/"+content.msgid+".amr")
				filebox.path = "."
				return filebox
			case "voip_doc_share":
				var filename2 = content.voip_doc_share.filename
				result = libSDK.getMedia(content.voip_doc_share.filesize,content.voip_doc_share.sdkfileid, 
				"cache/"+filename2, "None");
				filebox = FileBox.fromFile("cache/"+filename2)
				filebox.path = "."
				return filebox
			default:
    ...
    messageImage(messageId: string, imageType: ImageType): Promise<FileBox> {
        throw new Error("messageImage Method not implemented.")
    }
    messageMiniProgram(messageId: string): Promise<MiniProgramPayload> {
        throw new Error("messageMiniProgram Method not implemented.")
    }
    messageUrl(messageId: string): Promise<UrlLinkPayload> {
        throw new Error("messageUrl Method not implemented.")
    }
    ...
    public async messageRawPayloadParser(rawPayload: MessagePayload): Promise<MessagePayload> {
		...
		switch (content.msgtype){
			...
			case "card":
				payload.type = MessageType.Text
				payload.text = "名片。 corpname: "+content.card.corpname+" ; userid: "+content.card.userid
				break
			case "revoke":
				payload.type = MessageType.Text
				payload.text = "revoke a message. The origianl message id is"+content.revoke.pre_msgid
				break
			case "location":
				payload.type = MessageType.Location
				payload.text = "location. "+JSON.stringify(content.location)
				break
			case "link":
				payload.type = MessageType.Url
				payload.text = "link. "+JSON.stringify(content.link)
				break
			case "weapp":
				payload.type = MessageType.MiniProgram
				payload.text = "小程序. "+JSON.stringify(content.weapp)
				break
			case "chatrecord":
				payload.type = MessageType.Text
				payload.text = "聊天记录。 "+JSON.stringify(content.chatrecord)
				break
			case "todo":
				payload.type = MessageType.Text
				payload.text = "待办. "+JSON.stringify(content.todo)
				break
			case "image":
			...
			case "agree":
				payload.type = MessageType.Text
				payload.text = "同意会话聊天内容。user "+content.agree.userid+" agrees"
				break
			case "disagree":
				payload.type = MessageType.Text
				payload.text = "不同意会话聊天内容。user "+content.disagree.userid+" disagrees"
				break
			case "vote":
				payload.type = MessageType.Text
				payload.text = "投票. "+JSON.stringify(content.vote)
			case "collect":
				payload.type = MessageType.Text
				payload.text = "填表消息. "+JSON.stringify(content.collect)
			case "redpacket":
				payload.type = MessageType.Text
				payload.text = "红包消息. "+JSON.stringify(content.redpacket)
			case "meeting":
				payload.type = MessageType.Text
				payload.text = "会议邀请消息. "+JSON.stringify(content.meeting)
			case "docmsg":
				payload.type = MessageType.Text
				payload.text = "在线文档消息. "+JSON.stringify(content.docmsg)
			case "news":
				payload.type = MessageType.Text
				payload.text = "图文消息. "+JSON.stringify(content.news)
			case "calendar":
				payload.type = MessageType.Text
				payload.text = "日程消息. "+JSON.stringify(content.calendar)
			case "mixed":
				payload.type = MessageType.Text
				payload.text = "混合消息. "+JSON.stringify(content.mixed)
			case "meeting_voice_call":
				payload.type = MessageType.Audio
				payload.text = "音频存档消息. "+" filedata: "+JSON.stringify(content.meeting_voice_call.demofiledata)+
				" 共享屏幕data: "+JSON.stringify(content.meeting_voice_call.sharescreendata)
			case "voip_doc_share":
				payload.type = MessageType.Attachment
			default:
				payload.type = MessageType.Unknown
				payload.text = "Sorry, we don't support this kind of message right now. The message is of type "+content.msgtype
		}
    ...
    contactSelfQRCode(): Promise<string> {
        throw new Error("contactSelfQRCode Method not implemented.")
    }
    ...
    tagContactAdd(tagId: string, contactId: string): Promise<void> {
        throw new Error("tagContactAdd Method not implemented.")
    }
    tagContactDelete(tagId: string): Promise<void> {
        throw new Error("tagContactDelete Method not implemented.")
    }
    ...
    friendshipAccept(friendshipId: string): Promise<void> {
        throw new Error("friendshipAccept Method not implemented.")
    }
    friendshipAdd(contactId: string, hello?: string): Promise<void> {
        throw new Error("friendshipAdd Method not implemented.")
    }
    friendshipSearchPhone(phone: string): Promise<string> {
        throw new Error("friendshipSearchPhone Method not implemented.")
    }
    friendshipSearchWeixin(weixin: string): Promise<string> {
        throw new Error("friendshipSearchWeixin Method not implemented.")
    }
    ...
    messageSendFile(conversationId: string, file: FileBox): Promise<string | void> {
        throw new Error("messageSendFile Method not implemented.")
    }
    messageSendMiniProgram(conversationId: string, miniProgramPayload: MiniProgramPayload): Promise<string | void> {
        throw new Error(" messageSendMiniProgram Method not implemented.")
    }
    ...
    messageSendUrl(conversationId: string, urlLinkPayload: UrlLinkPayload): Promise<string | void> {
        throw new Error("messageSendUrl Method not implemented.")
    }
    messageRecall(messageId: string): Promise<boolean> {
        throw new Error("messageRecall Method not implemented.")
    }

    roomInvitationAccept(roomInvitationId: string): Promise<void> {
        throw new Error("roomInvitationAccept Method not implemented.")
    }
    protected roomInvitationRawPayload(roomInvitationId: string): Promise<any> {
        throw new Error("roomInvitationRawPayload Method not implemented.")
    }
    protected roomInvitationRawPayloadParser(rawPayload: any): Promise<import("wechaty-puppet").RoomInvitationPayload> {
        throw new Error("roomInvitationRawPayloadParser Method not implemented.")
    }
    roomAdd(roomId: string, contactId: string): Promise<void> {
        throw new Error("roomAdd Method not implemented.")
    }
    roomAvatar(roomId: string): Promise<FileBox> {
        throw new Error("roomAvatar Method not implemented.")
    }
    roomCreate(contactIdList: string[], topic?: string): Promise<string> {
        throw new Error("roomCreate Method not implemented.")
    }
    roomDel(roomId: string, contactId: string): Promise<void> {
        throw new Error("roomDel Method not implemented.")
    }
    roomList(): Promise<string[]> {
        throw new Error("roomList Method not implemented.")
    }
    roomQRCode(roomId: string): Promise<string> {
        throw new Error("roomQRCode Method not implemented.")
    }
    roomQuit(roomId: string): Promise<void> {
        throw new Error("roomQuit Method not implemented.")
    }
    ...
    roomAnnounce(roomId: any, text?: any): Promise<string> | Promise<void> {
        throw new Error("roomAnnounce Method not implemented.")
    }
